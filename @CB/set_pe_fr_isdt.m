function obj = set_pe_fr_isdt(obj, save, varargin)
% PE_FR_ISDT logical for availability of date (year, month, day) of
% ba funding round
%
% DESCR
...
n     = 11;
name  = 'pe_fr_isdt';
descr = 'people fr --> pe_fr_isdt: logical for availability of date (year, month, day) of business angel funding round';
input = {'pe_fr_dtfr',...
  'pe_ninv'};
...
%% compute
pe_fr_isdt = cell(size(pe_fr_dtfr));         % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:size(pe_fr_dtfr{bas(ba)},1)
    
    if isempty(pe_fr_dtfr{bas(ba)}{fr,1})
      pe_fr_isdt{bas(ba)}(fr,1) = false;
    else
      pe_fr_isdt{bas(ba)}(fr,1) = true;
    end
    
    if isempty(pe_fr_dtfr{bas(ba)}{fr,2})
      pe_fr_isdt{bas(ba)}(fr,2) = false;
    else
      pe_fr_isdt{bas(ba)}(fr,2) = true;
    end
    
    if isempty(pe_fr_dtfr{bas(ba)}{fr,3})
      pe_fr_isdt{bas(ba)}(fr,3) = false;
    else
      pe_fr_isdt{bas(ba)}(fr,3) = true;
    end
