function obj = set_pe_fr_sqn(obj, save, varargin)
% PE_FR_SQN  sequence number of business angel funding round of all
...
n     = 33;
name  = 'pe_fr_sqn';
descr = 'ba -> ba fr -> pe_fr_sqn: sequence number of business angel funding round of all company funding rounds';
...
%% compute
pe_fr_sqn = cell(size(pe_ninv));   % preallocate

bas = find(pe_ninv>0);                      % all bas
for ba = 1:length(bas)                        % ba
  for bafr = 1:length(pe_fr_comfr_sqn{bas(ba)})      % ba fr
    
    pe_fr_sqn{bas(ba)}(bafr)...
      = pe_fr_comfr_sqn{bas(ba)}{bafr}(pe_fr_bafrind{bas(ba)}(bafr));
