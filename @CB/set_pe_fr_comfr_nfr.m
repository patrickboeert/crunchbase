function obj = set_pe_fr_comfr_nfr(obj, save, varargin)
% PE_FR_COMFR_NFR  number of funding rounds for companies
% in the funding rounds of the business angel
%
% DESCR
...
%% parameters
n     = 22;
name  = 'pe_fr_comfr_nfr';
descr = 'ba -> ba fr -> pe_fr_comfr_nfr: number of funding rounds for companies in the funding rounds of the business angel';
...
%% compute
pe_fr_comfr_nfr = cell(size(pe_ninv));           % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr_comind{bas(ba)})
    pe_fr_comfr_nfr{bas(ba)}(fr) = length(funding_rounds{pe_fr_comind{bas(ba)}(fr)});
  end
end
