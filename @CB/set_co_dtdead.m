function obj = set_co_dtdead(obj, save, varargin)
% CO_DTDEAD  deadpool date (year, month, day) for company
%
% DESCR
...
n     = 20;
name  = 'co_dtdead';
descr = 'com --> co_dtdead: deadpool date (year, month, day) for company';
input = {'companies_deadpooled_year',...
  'companies_deadpooled_month'};
...
%% compute
co_dtdead = cell(size(deadpooled_year));
for com = 1:length(co_dtdead)
  
  if isnan(deadpooled_year(com))
    co_dtdead{com}{1} = [];
  else
    co_dtdead{com}{1} = deadpooled_year(com);
  end
  
  if isnan(deadpooled_month(com))
    co_dtdead{com}{2} = [];
  else
    co_dtdead{com}{2} = deadpooled_month(com);
  end
