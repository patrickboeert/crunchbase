function obj = set_pe_fr_dtipo(obj, save, varargin)
% PE_FR_DTIPO date (year, month, day) of IPO event for business angel
...
n     = 15;
name  = 'pe_fr_dtipo';
descr = 'people fr --> pe_fr_dtipo: date (year, month, day) of IPO event for business angel funding round';
input = {'pe_ninv',...
  'pe_fr_ipo'};
...
%% compute
pe_fr_dtipo = cell(size(pe_fr_ipo));
...
for ba = 1:length(bas)
  for fr = 1:length(pe_fr_ipo{bas(ba)})
    ...
    if isempty(pe_fr_ipo{bas(ba)}{fr})
      pe_fr_dtipo{bas(ba)}{fr,1} = [];
      pe_fr_dtipo{bas(ba)}{fr,2} = [];
      pe_fr_dtipo{bas(ba)}{fr,3} = [];
    else
      pe_fr_dtipo{bas(ba)}{fr,1} = pe_fr_ipo{bas(ba)}{fr}.pub_year;
      pe_fr_dtipo{bas(ba)}{fr,2} = pe_fr_ipo{bas(ba)}{fr}.pub_month;
      pe_fr_dtipo{bas(ba)}{fr,3} = pe_fr_ipo{bas(ba)}{fr}.pub_day;
    end
