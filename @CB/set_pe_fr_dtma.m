function obj = set_pe_fr_dtma(obj, save, varargin)
% PE_FR_DTMA date (year, month, day) of M&A event for business angel
% company in the funding round
%
% DESCR
...
n     = 18;
name  = 'pe_fr_dtma';
descr = 'people fr --> pe_fr_dtma: date (year, month, day) of M&A event for business angel funding round';
input = {'pe_ninv',...
  'pe_fr_ma'};
...
for ba = 1:length(bas)
  for fr = 1:length(pe_fr_ma{bas(ba)})
    ...
    if isempty(pe_fr_ma{bas(ba)}{fr})
      pe_fr_dtma{bas(ba)}{fr,1} = [];
      pe_fr_dtma{bas(ba)}{fr,2} = [];
      pe_fr_dtma{bas(ba)}{fr,3} = [];
    else
      pe_fr_dtma{bas(ba)}{fr,1} = pe_fr_ma{bas(ba)}{fr}.acquired_year;
      pe_fr_dtma{bas(ba)}{fr,2} = pe_fr_ma{bas(ba)}{fr}.acquired_month;
      pe_fr_dtma{bas(ba)}{fr,3} = pe_fr_ma{bas(ba)}{fr}.acquired_day;
    end
