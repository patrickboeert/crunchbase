function obj = set_pe_fr_comind(obj, save, varargin)
% PE_FR_COMIND company index of ba funding round company
%
% DESCR
...
n     = 12;
name  = 'pe_fr_comind';
descr = 'map: ba fr -> company index of ba funding round company';
input = {'pe_fr_comperma',...     % company permalink in ba fr
  'pe_ninv',...            % pe selection
  'companies_permalink'};  % all company permalinks
...
%% compute
pe_fr_comind = cell(size(pe_ninv));  % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr_comperma{bas(ba)})
    
    % find matching company index
    cmp = strcmp(pe_fr_comperma{bas(ba)}{fr}, permalink);
    
    % assign value, error if dim > 1 = more than one match
    if sum(cmp) ~= 1
      pe_fr_comind{bas(ba)}(fr) = find(cmp, 1, 'first');
    else
      pe_fr_comind{bas(ba)}(fr) = find(cmp);
    end
