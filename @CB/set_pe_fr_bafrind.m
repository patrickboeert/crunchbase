function obj = set_pe_fr_bafrind(obj, save, varargin)
% PE_FR_BAFRIND  index of business angel funding round in
% company funding round structure
%
% DESCR
...
n     = 31;
name  = 'pe_fr_bafrind';
descr = 'ba -> ba fr -> pe_fr_bafrind: index of business angel funding round in company funding round structure';
input = {'pe_fr_comfr',...                 % company funding rounds
  'pe_ninv',...                     % pe selection
  'pe_fr_comfr_isbafr'};            % is ba fr?
...
%% compute
pe_fr_bafrind = cell(size(pe_ninv));   % preallocate

bas = find(pe_ninv>0);                      % all bas
for ba = 1:length(bas)                        % ba
  for bafr = 1:length(pe_fr_comfr{bas(ba)})      % ba fr
    
    pe_fr_bafrind{bas(ba)}(bafr)...
      = find(pe_fr_comfr_isbafr{bas(ba)}{bafr}(:));
