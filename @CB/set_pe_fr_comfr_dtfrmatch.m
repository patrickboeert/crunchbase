function obj = set_pe_fr_comfr_dtfrmatch(obj, save, varargin)
% PE_FR_COMFR_DTFRMATCH  logical for: the date of the business angel
% funding round matches with the date of the company funding round
%
% DESCR
...
%% parameters
n     = 29;
name  = 'pe_fr_comfr_dtfrmatch';
descr = 'ba -> ba fr -> com fr --> pe_fr_comfr_dtfrmatch: logical for: the date of the business angel funding round matches with the date of the company funding round';
input = {'pe_fr_comfr',...                 % company funding rounds
  'pe_ninv',...                     % pe selection
  'pe_fr_dtfr',...                  % dt of ba fr
  'pe_fr_comfr_dtcomfr'};           % dt of com fr
...
%% compute
pe_fr_comfr_dtfrmatch = cell(size(pe_ninv));   % preallocate
...
for ba = 1:length(bas)                        % ba
  for bafr = 1:length(pe_fr_comfr{bas(ba)})      % ba fr
    for comfr = 1:length(pe_fr_comfr{bas(ba)}{bafr}) % com fr
      ...
      % dt for the com fr
      badt = pe_fr_dtfr{bas(ba)}(bafr,1:2);
      codt = pe_fr_comfr_dtcomfr{bas(ba)}{bafr}(comfr,1:2);
      
      % check for consistency of year & month
      for i=[1,2]
        if isempty(badt(i)) || isempty(codt(i))   % one or two empty
          if isempty(badt(i)) && isempty(codt(i))  % both empty
            match(i) = true;
          else
            match(i) = false;
          end
      ...
      % both year & month need to be consistent
      if all(match)
        pe_fr_comfr_dtfrmatch{bas(ba)}{bafr}(comfr) = true;
      else
        pe_fr_comfr_dtfrmatch{bas(ba)}{bafr}(comfr) = false;
      end
...
end % function PE_FR_COMFR_DTFRMATCH