function obj = set_com_fr_ninv(obj, save, varargin)
% COM_FR_NINV  number of identifiable investors in the funding
% rounds of the companies
%
% DESCR
...
n     = 23;
name  = 'com_fr_ninv';
descr = 'com --> fr --> com_fr_ninv: number of identifiable investors in the funding rounds of the companies';
...
com_fr_ninv = cell(size(comfr));           % preallocate

for com = 1:length(comfr)
  for fr = 1:length(comfr{com}) % com fr
    
    if isempty(comfr{com}(fr).investments)
      com_fr_ninv{com}(fr) = 0;
    else
      com_fr_ninv{com}(fr) = size(comfr{com}(fr).investments,2);
    end
