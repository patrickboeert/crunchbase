function obj = set_pe_fr_roundcode(obj, save, varargin)
% PE_FR_ROUNDCODE round code of bas funding round
%
% DESCR
...
%% parameters
n     = 6;
name  = 'pe_fr_roundcode';
descr = 'people fr --> pe_fr_roundcode: round code of ba funding round';
input = {'pe_ninv',...
  'people_investments'};
...
pe_fr_roundcode = cell(size(investments));  % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr{bas(ba)})
    pe_fr_roundcode{bas(ba)}{fr} = pe_fr{bas(ba)}(fr).funding_round.round_code;
  end
end
