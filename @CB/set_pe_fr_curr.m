function obj = set_pe_fr_curr(obj, save, varargin)
% PE_FR_CURR currency code in business angel funding round
%
% DESCR
...
n     = 9;
name  = 'pe_fr_curr';
descr = 'people fr --> pe_fr_curr: currency code in funding round';
input = {'pe_ninv',...
  'people_investments'};
...
pe_fr_curr   = cell(size(investments));  % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr{bas(ba)})
    pe_fr_curr{bas(ba)}{fr} = pe_fr{bas(ba)}(fr).funding_round.raised_currency_code;
  end
end
