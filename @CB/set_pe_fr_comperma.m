function obj = set_pe_fr_comperma(obj, save, varargin)
% PE_FR_COMPERMA pe_fr_comperma: company permalink of bas portfolio
% companies
%
% DESCR
...
n     = 5;
name  = 'pe_fr_comperma';
descr = 'people fr --> com permalink: company permalink of bas portfolio companies';
input = {'pe_ninv',...
  'people_investments'};
...
%% compute, given: pe_ninv, investments
pe_fr          = investments;              % all pe frs
pe_fr_comperma = cell(size(investments));  % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr{bas(ba)})
    pe_fr_comperma{bas(ba)}{fr} = pe_fr{bas(ba)}(fr).funding_round.company.permalink;
  end
end
