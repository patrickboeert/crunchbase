function obj = set_pe_fr_ipo(obj, save, varargin)
% PE_FR_IPO  IPO events for business angel company in the funding round
%
% DESCR
...
n     = 13;
name  = 'pe_fr_ipo';
descr = 'ba fr -> IPO events for business angel companies in the funding round';
...
%% compute (pe_fr_comind, pe_ninv, ipo)
pe_fr_ipo = cell(size(pe_ninv));           % preallocate

bas = find(pe_ninv>0);                     % all bas
for ba = 1:length(bas)
  for fr = 1:length(pe_fr_comind{bas(ba)})
    pe_fr_ipo{bas(ba)}{fr} = ipo{pe_fr_comind{bas(ba)}(fr)};
  end
end
